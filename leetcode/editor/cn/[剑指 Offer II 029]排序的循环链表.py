...
class Solution:
    def insert(self, head: 'Node', insertVal: int) -> 'Node':
        ...
        #     node.next = head
        # else:
        #     def insertCore(head: 'Node', node: 'Node'):
        #         # 先找两个节点
        #         cur = head
        #         nxt = head.next
        #         biggest = head
        #         while not(cur.val <= node.val <= nxt.val) and nxt != head:
        #             cur = nxt
        #             nxt = nxt.next
        ...
        while nxt != head:
            if curr.val <= insertVal <= nxt.val:
                break
            # 因为是有序（从小到大）链表，所以curr -> nxt.
            # 当curr.val > nxt.val 说明curr.val是当前链表的最大值,nxt.val是当前链表的最小值。
            if curr.val > nxt.val:
                # insertVal大于最大值，或者小于最小值。
